...
import IlchonManageModal from '../home/IlchonManageModal';
...
function Header() {
    ...
    const handleRandomVisit = async () => {
        try {
            ...
            if (randomUserLoginId) {
                navigate(`/${randomUserLoginId}`);
            } else {
                alert('방문할 사용자를 찾지 못했습니다.');
            }
        } catch (error) {
            console.error("랜덤 사용자를 불러오는 데 실패했습니다.", error);
    ...
    return (
        <>
                    ...
                    <nav className="site-nav">
                        {currentUser ? (
                            <>
                                ...
                                <button onClick={() => setIsIlchonModalOpen(true)}>일촌 관리</button>
            ...
            {isIlchonModalOpen && 
                <IlchonManageModal 
                    onClose={() => setIsIlchonModalOpen(false)} 
                    initialTab="myIlchons"
                    onUpdatePage={handleModalUpdate}
                />
            }
        </>
    );
